...
export function QuestionItem({ question, userInfo, onUpdateLikes }: QuestionItemProps) {
  ...
  const initials = useMemo(() => getInitials(question.author.userName), [question.author.userName])
  const backgroundColor = useMemo(() => getSubtleColor(question.author.userName), [question.author.userName])
    ...
    <div className="flex items-start gap-4 p-4 border rounded-lg">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <h3 className="font-semibold text-base">{question.author.userName}</h3>
      ...
      <Button
        variant="ghost"
        size="sm"
        className={`gap-1 ${isLiked ? "bg-primary/10 text-primary hover:bg-primary/20" : ""}`}
        onClick={handleLike}
      >
